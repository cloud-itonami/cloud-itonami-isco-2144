(ns mechanical-engineering.actor
  "MechanicalEngineeringActor — the ISCO-08 2144 independent
  mechanical-engineering actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  engineering operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the DesignAdvisor can never directly
  commit a record or dispatch a robot action the
  MechanicalEngineeringGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mechanical-engineering.advisor :as advisor]
            [mechanical-engineering.governor :as governor]
            [mechanical-engineering.store :as store]))

(defn build-graph
  "Build a compiled MechanicalEngineeringActor graph. `store`
  implements `mechanical-engineering.store/Store`. `advisor`
  implements `mechanical-engineering.advisor/Advisor` (defaults to
